...
handle_log_close(int handle, char *emsg)
{
	if (handle_is_ok(handle, HANDLE_FILE)) {
		...
		if (alu_is_cc_enable())
			logit("EVENT-AUDIT: %s%sclose \"%s\" bytes read %llu written %llu",
...
#endif
			debug("%s%sclose \"%s\" bytes read %llu written %llu",
				emsg == NULL ? "" : emsg, emsg == NULL ? "" : " ",
				handle_to_name(handle),
				(unsigned long long)handle_bytes_read(handle),
				(unsigned long long)handle_bytes_write(handle));
...
#ifdef ALU_CC_CAPABLE
		if (alu_is_cc_enable())
			logit("EVENT-AUDIT: %s%sclosedir \"%s\"",
				emsg == NULL ? "" : emsg, emsg == NULL ? "" : " ",
				handle_to_name(handle));
		else
#endif
...
#ifdef ALU_CC_CAPABLE
		if (alu_is_cc_enable())
			logit("EVENT-AUDIT: sent status %s", status_to_message(status));
		else
#endif
#ifdef ALU_JITC_CAPABLE
		if (alu_is_jitc_enable())
...
#endif
			debug("sent status %s", status_to_message(status));
	buffer_init(&msg);
...
#ifdef ALU_CC_CAPABLE
	if (alu_is_cc_enable())
		logit("EVENT-AUDIT: open \"%s\" flags %s mode 0%o",
...
#ifdef ALU_CC_CAPABLE
		if (alu_is_cc_enable())
			logit("EVENT-AUDIT: set \"%s\" size %llu",
...
#ifdef ALU_CC_CAPABLE
		if (alu_is_cc_enable())
			logit("EVENT-AUDIT: set \"%s\" mode %04o", name, a->perm);
		else
#endif
#ifdef ALU_JITC_CAPABLE
		if (alu_is_jitc_enable())
...
#endif
			debug("set \"%s\" mode %04o", name, a->perm);
...
#ifdef ALU_CC_CAPABLE
		if (alu_is_cc_enable())
			logit("EVENT-AUDIT: set \"%s\" modtime %s", name, buf);
		else
#endif
#ifdef ALU_JITC_CAPABLE
		if (alu_is_jitc_enable())
...
#endif
			debug("set \"%s\" modtime %s", name, buf);
...
#ifdef ALU_CC_CAPABLE
		if (alu_is_cc_enable())
			logit("EVENT-AUDIT: set \"%s\" owner %lu group %lu", name,
...
#endif
			debug("set \"%s\" owner %lu group %lu", name,
				(u_long)a->uid, (u_long)a->gid);
...
#ifdef ALU_CC_CAPABLE
	if (alu_is_cc_enable())
		logit("EVENT-AUDIT: opendir \"%s\"", path);
	else
#endif
#ifdef ALU_JITC_CAPABLE
	if (alu_is_jitc_enable())
...
#ifdef ALU_CC_CAPABLE
	if (alu_is_cc_enable())
		logit("EVENT-AUDIT: remove name \"%s\"", name);
	else
#endif
#ifdef ALU_JITC_CAPABLE
	if (alu_is_jitc_enable())
...
#ifdef ALU_CC_CAPABLE
	if (alu_is_cc_enable())
		logit("EVENT-AUDIT: mkdir name \"%s\" mode 0%o", name, mode);
	else
#endif
#ifdef ALU_JITC_CAPABLE
	if (alu_is_jitc_enable())
...
#endif
		debug("mkdir name \"%s\" mode 0%o", name, mode);
	if (readonly)
		status = SSH2_FX_PERMISSION_DENIED;
	else {
		ret = mkdir(name, mode);
...
#ifdef ALU_CC_CAPABLE
	if (alu_is_cc_enable())
		logit("EVENT-AUDIT: rmdir name \"%s\"", name);
	else
#endif
#ifdef ALU_JITC_CAPABLE
	if (alu_is_jitc_enable())
...
#ifdef ALU_CC_CAPABLE
	if (alu_is_cc_enable())
		logit("EVENT-AUDIT: rename old \"%s\" new \"%s\"", oldpath, newpath);
	else
#endif
#ifdef ALU_JITC_CAPABLE
	if (alu_is_jitc_enable())
...
#endif
		debug("rename old \"%s\" new \"%s\"", oldpath, newpath);
	status = SSH2_FX_FAILURE;
	if (readonly)
		status = SSH2_FX_PERMISSION_DENIED;
...
#ifdef ALU_CC_CAPABLE
	if (alu_is_cc_enable())
		logit("EVENT-AUDIT: symlink old \"%s\" new \"%s\"", oldpath, newpath);
	else
#endif
#ifdef ALU_JITC_CAPABLE
	if (alu_is_jitc_enable())
...
#endif
		debug("symlink old \"%s\" new \"%s\"", oldpath, newpath);
...
#ifdef ALU_CC_CAPABLE
	if (alu_is_cc_enable())
		logit("EVENT-AUDIT: posix-rename old \"%s\" new \"%s\"", oldpath, newpath);
	else
#endif
#ifdef ALU_JITC_CAPABLE
	if (alu_is_jitc_enable())
...
#endif
		debug("posix-rename old \"%s\" new \"%s\"", oldpath, newpath);
	if (readonly)
		status = SSH2_FX_PERMISSION_DENIED;
	else {
...
#ifdef ALU_CC_CAPABLE
	if (alu_is_cc_enable())
		logit("EVENT-AUDIT: statfs \"%s\"", path);
	else
#endif
#ifdef ALU_JITC_CAPABLE
	if (alu_is_jitc_enable())
...
#ifdef ALU_CC_CAPABLE
	if (alu_is_cc_enable())
		logit("EVENT-AUDIT: hardlink old \"%s\" new \"%s\"", oldpath, newpath);
	else
#endif
#ifdef ALU_JITC_CAPABLE
	if (alu_is_jitc_enable())
...
#endif
		debug("hardlink old \"%s\" new \"%s\"", oldpath, newpath);
	if (readonly)
		status = SSH2_FX_PERMISSION_DENIED;
	else {
...
#ifdef ALU_CC_CAPABLE
		if (alu_is_cc_enable())
			logit("EVENT-AUDIT: session closed for local user %s from [%s]",
...
#endif
			debug("session closed for local user %s from [%s]",
				pw->pw_name, client_addr);
	}
	_exit(i);
}
...
#ifdef ALU_CC_CAPABLE
	if (alu_is_cc_enable())
		logit("EVENT-AUDIT: session opened for local user %s from [%s]",
			pw->pw_name, client_addr);
	else
#endif
#ifdef ALU_JITC_CAPABLE
...
#endif
		debug("session opened for local user %s from [%s]",
			pw->pw_name, client_addr);
