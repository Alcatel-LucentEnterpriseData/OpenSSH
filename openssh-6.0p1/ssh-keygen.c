 ...
 *
 * Copyright 2019 ALE USA Inc. Derived Version.
 *
 *                    All rights reserved
...
#define FIPS_FILE_LOCATION "/var/run/fips.enable"
...
type_bits_valid(int type, u_int32_t *bitsp)
{
	...
	if (type == KEY_DSA) {
                if (FIPS_mode()) {
                        if (*bitsp != 2048)
                               fatal("DSA keys must be 2048 bits");
                }
                else {
                        if (*bitsp != 1024) 
		               fatal("DSA keys must be 1024 bits");
...
main(int argc, char **argv)
{
	 ...
	 */
	// ALU: Always call ssh-keygen in fips mode
	// if (getenv("OPENSSL_FIPS")) {
        struct stat fileStat;

        if( stat(FIPS_FILE_LOCATION, &fileStat) == 0 && FIPS_mode() == 0) {
	  if (!FIPS_mode_set(1)) {
